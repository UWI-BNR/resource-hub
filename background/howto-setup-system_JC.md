...
???- example "Step 2 - Install Git and Connect to GitHub"
    ...
    ### Verification:  
    In the folder you chose, you should see subfolders such as `do`, `ado`==Ian, I only see these 2 subfolders at this step so maybe strikethrough these ones as they are only discoverable in Step 4...?==~~, `tables`, `graphics`, `outputs`, and `temp`~~.
...
???- example "Step 4 — Check your Analytics Folder Structure"
    ...
    │   ├── setup       ==JC added this extra line as subfolder 'setup' was missing ==
    │   ├── tables
    │   ├── temp
    ├── .gitignore
    ├── README.md
...
???- example "Step 5 — Configure profile.do in Stata"
    ...
    14. Note the folder shown next to `PERSONAL`. ==*You may have to create this folder; simply use the New Folder option on your PC*==
...
???- example "Step 7 — Run a Simple Test DO File"
    ...
    4.  ==Stata version 19 should be used, otherwise you may need to install certain user-written packages, e.g. colorpalette.==
...
???- example "Step 8 — Run an Analytics Script as a Final Check"
    ...
    4.  ==**Note:** If you do not have the font 'Montserrat' installed on your PC, you will need to perform the below steps:==
        1.  Download font files from: [https://fonts.google.com/specimen/Montserrat](https://fonts.google.com/specimen/Montserrat){ target="_blank"}
        2.  Unzip the files
