...
???- info "(8) Setting up the system on a local PC"

    ## (8) Step-by-Step Setup
    
    ### Preparing Your Local PC for the BNR Analytics System

    This section brings together all the previous steps into one simple checklist, so you can prepare your PC for the BNR Analytics System and verify that everything works.
    ...
    6. In the search bar, type `Stata Enhanced`.
    ...
    8. Go to https://git-scm.com/downloads.
    9. Download and install Git for your operating system (accept default options).
    10. Create a free GitHub account at https://github.com if you do not already have one.
    11. Open VS Code.
    12. Click the Accounts icon in the bottom-left corner and choose the option to sign in to GitHub.
    13. Authorise VS Code to access your GitHub account when prompted.

    To download (clone) the BNR Analytics repository:

    14. In VS Code, open the Command Palette (Ctrl+Shift+P).
    15. Type `Git: Clone` and press Enter.
    16. Paste the repository URL: `https://github.com/UWI-BNR/bnr-analytics`
    17. Choose a folder on your PC where you want the repository to live.
    18. Wait for the download to complete, then open the newly downloaded folder in VS Code.
    ...
    ### Step 3 — Install or Update Python (Version 3.x Required)

    19. In VS Code, open the Terminal (Ctrl+Shift+`).
    20. Type `python --version` and press Enter.
    21. If this shows Python 3.x, you can keep this version.  
    If it shows Python 2.x, or gives an error, install Python 3:

    a. Go to https://www.python.org/downloads/  
    b. Download the latest Python 3 version.  
    c. Run the installer and tick the box `Add Python to PATH`.  
    d. Click `Install Now` and complete the setup.

    22. After installation, open a new Terminal in VS Code and run `python --version` again to confirm it shows Python 3.x.

    Verification inside Stata:

    23. Open Stata.
    24. Type `python query` and press Enter.
    25. Confirm that the version is Python 3.x and a valid executable path is shown.

    If Stata does not show the correct path, see Instruction 3 for how to set it using `python set exec`.

    ---

    ### Step 4 — Check the Analytics Folder Structure
    ...
    You will also have a link to a ZIP file (`bnr-analytics.zip`). This zip file contains the remaining subfolders. 

    26. Place the zip file in the `bnr-analytics/` subfolder.
    27. Unzip to bnr-analytics/ (NOT TO a subfolder)
    28. Once unzipped, your `bnr-analytics` folder should look like this:
    ...
    29. Open Stata and type `sysdir` in the Command window, then press Enter.
    30. Note the folder shown next to `PERSONAL`.
    31. In that `PERSONAL` folder, create or edit the file `profile.do`.
    32. Add lines that tell Stata where your `bnr-analytics` repository lives and where Python is installed.
    ...
    34. Close Stata completely.
    35. Open Stata again (this runs `profile.do`).
    36. Type `adopath` and press Enter. Confirm your `bnr-analytics` paths are listed.
    37. Type `python query` and press Enter. Confirm the Python path and version are correct.

    ---

    ### Step 6 — Run a Simple Test DO File

    38. In Stata, use the `cd` command to move to your `bnr-analytics` folder, or supply the full path in quotes.
    39. Run the global setup file, for example:

    `do "C:/Users/Joan/BNR/bnr-analytics/bnrcvd-globals.do"`

    40. Check that it runs without errors.

    ---

    ### Step 7 — Run an Analytics Script as a Final Check

    41. In Stata, run one of the main analytics scripts, for example:

    `do "C:/Users/Joan/BNR/bnr-analytics/bnrcvd-2023-count.do"`

    42. Watch the Stata output window for any missing-file or path errors.
    43. When it finishes, look in the `outputs` or `tables` folder to confirm that new output files (for example Excel or PDF) have been created.
